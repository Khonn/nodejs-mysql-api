                                        ...
                                        if(result && result.length){
                                            ...
                                            con.query('select count(*) as count from title_details where title_id=?',[title_id],function(err,result){
                                                if(result && result.length){
                                                    ...
                                                    con.query('delete from title_details where title_id=?',[title_id]);
                ...
                con.query('select collection_id from user_collection where user_email=?',[email],function(err,result,fields){
                    ...
                    if(result && result.length){
                        ...
                        con.query('select title_id from collection_overview where collection_id=? and title_name=?',[collection_id,title_name],function(err,result,fields){
                            ...
                            con.query('insert into title_details (title_id, entry_name, page, text_scanned, text_simplified) values(?,?,?,?,?)',[title_id, entry_name, page, text_scanned, text_simplified],function(err,result,fields){
                                    res.send(JSON.stringify(result));
                            });
                            }
                        })
